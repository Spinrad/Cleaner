...
def _dk(**kw):
    """Helper: create DerivedParams from keyword mappings."""
    d = DerivedParams()
    for k, v in kw.items():
        if hasattr(d, k):
            setattr(d, k, v)
...
def test_build_graph_structure():
    d = _dk(comp_threshold_linear=0.18, comp_ratio=4, comp_attack_ms=2.0,
...
def test_notch_frequencies_in_graph():
    d = _dk(comp_threshold_linear=0.05, comp_ratio=4, comp_attack_ms=2.0,
...
def test_sidechain_structure():
    d = _dk(comp_threshold_linear=0.05, comp_ratio=4, comp_attack_ms=2.0,
            ...
            expander_threshold_linear=0.05, expander_ratio=2.0,
            expander_attack_ms=5.0, expander_release_ms=40.0,
            sat_threshold_linear=0.85, air_db=0.0, width=0.0,
            bus_threshold_linear=0.18, bus_ratio=2, bus_attack_ms=10,
            bus_release_ms=100, bus_mix=0.0)
    g = build_filtergraph(derived=d)
    assert "[side]" in g
...
def test_hp_disabled():
    ...
    g = build_filtergraph(stages=stages, derived=d)
    assert "highpass" not in g
    assert "stereotools" in g
...
def test_mastering_flags():
    d = _dk(comp_threshold_linear=0.18, comp_ratio=4, comp_attack_ms=2.0,
            ...
            expander_range_linear=0.12, sat_threshold_linear=0.85,
            air_db=2.0, width=0.3,
            bus_threshold_linear=0.18, bus_ratio=2, bus_attack_ms=10,
            bus_release_ms=100, bus_mix=0.0)
...
def test_bus_comp_enabled():
    d = _dk(comp_threshold_linear=0.18, comp_ratio=4, comp_attack_ms=2.0,
            ...
            expander_threshold_linear=0.4, expander_ratio=1.4,
            expander_attack_ms=1.0, expander_release_ms=15.0,
            expander_range_linear=0.12, sat_threshold_linear=0.95,
            air_db=0.0, width=0.0,
